...
//PID constants - glaaron dependent
float kp = glaaron ? 10. : 80.;
float ki = glaaron ? 0.005 : 0.05;
float kd = glaaron ? 8.5 : 250.;
//error ranges
#define firingErrorMargin 1.
...
int flywheelPower = 0;
float integral = 0;
int defaultPower = 0; //glaaron dependent
...
task calcVelocity() {
	while (true) {
		...
		flywheelVelocity = abs((float)(SensorValue[flywheelEncoder])) / (glaaron ? 1 : (float)(sampleTime)); //glaaron dependent
...
void setFlywheelRange(int range) {
	...
	float glaaronVelocities[5] = { 0, 106, 300, 183, 191 }; //glaaron dependent
	int setpowers[5] = {0, 70, 96, 65, 68}; //glaaron dependent


	defaultPower = setpowers[range];
	integral = 0;
	targetVelocity = glaaron ? glaaronVelocities[limit(range, 0, 4)] : velocities[limit(range, 0, 4)]; //glaaron dependent
...
task flywheelStabilization() { //modulates motor powers to maintain constant flywheel velocity
	...
	while (true)
	{
		while (abs(targetVelocity - flywheelVelocity) < bangBangErrorMargin * flywheelVelocity && targetVelocity > 0 || adjustmentPeriod || glaaron) //PID control - glaaron dependent
		{
			...
			setLauncherPower((glaaron ? defaultPower : 0) + kp * error + ki * integral + kd * (error - prevError) / sampleTime);
			prevError = error;
		}
