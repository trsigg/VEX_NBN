...
//autonomous region
task turnTask() {
	int startPoint = SensorValue[gyro]; //find initial gyro position
	//turn
	setDrivePower(sgn(degreesToTurn) * maxTurnSpeed, -sgn(degreesToTurn) * maxTurnSpeed);
	while (abs(SensorValue[gyro] - startPoint) < (degreesToTurn * 10)) {}
	//brake
...
void turn(float _degreesToTurn_, int _maxTurnSpeed_=40, bool runAsTask=false, int _waitAtEnd_=250) {
	...
	if (runAsTask) {
		...
		//turn
		setDrivePower(sgn(degreesToTurn) * maxTurnSpeed, -sgn(degreesToTurn) * maxTurnSpeed);
		while (abs(SensorValue[gyro] - startPoint) < abs(degreesToTurn * 10)) {}
		//brake
		setDrivePower(-sgn(degreesToTurn) * 10, sgn(degreesToTurn) * 10);
		int brakeDelay = limit(250, 0, waitAtEnd);
		wait1Msec(brakeDelay);
...
task autonomous() {
	...
	autonProgress = 1;
	turn(-90, 40, true); // turn toward net
	autonProgress = 2;
