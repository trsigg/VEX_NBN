...
void setDrivePower(int right, int left) {
	right = sgn(right) * limit(abs(right), 10, 127);
	left = sgn(left) * limit(abs(left), 10, 127);
	motor[rfdrive] = right;
	motor[rbdrive] = right;
	motor[lfdrive] = left;
	motor[lbdrive] = left;
...
//setFlywheelRange
int targetVelocity = 0; //also used in fire, feedControl, and flywheelStabilization
float firingErrorMargin; //also used in fire and feedControl
float setpointCoeff; //also used in flywheelStabilization

int velocities[4] = {0, 3.3, 5.1, 5.6};
float firingErrorMargins[4] = {0.1, 0.1, 0.1, 0.1};
float setpointCoeffs[4] = {1, 285714, 285714, 666666};
...
void setFlywheelRange(int range) {
	...
	setpointCoeff = setpointCoeffs[limitedRange];
}
...
//fire
float error; //also used in feedControl and flywheelStabilization
...
float bangBangErrorMargin=0.1;

//debug
int bangBangCount;
float avgError, bbpercentup, bangBangPerSec;
float batteryLevel;
...
task flywheelStabilization() { //modulates motor powers to maintain constant flywheel velocity
	...
	while (true)
	{
		while (/*abs(error) < bangBangErrorMargin * targetVelocity && targetVelocity > 0*/ flywheelVelocity >= targetVelocity) //battery level control
		{
			batteryLevel = nImmediateBatteryLevel;

			setLauncherPower(setpointCoeff/nImmediateBatteryLevel);
		}
		...
		bangBangPerSec = (float)bangBangCount * 1000.0 / (float)(time1(flywheelTimer) + .1);
...
task usercontrol() {
	initializeTasks(false);
	setFlywheelRange(0);
	int denominator = exp(driveExp * log(127));

	while (true)
	{
		while (vexRT[emergencyStopBtn] == 0) {
			if (vexRT[liftBtn] == 0) {
				setDrivePower(exp(driveExp * log(vexRT[Ch2])) / denominator, exp(driveExp * log(vexRT[Ch3])) / denominator);
			}
			else {
				setLauncherPower(-vexRT[Ch2], -127, 0);
			}
			EndTimeSlice();
		}
